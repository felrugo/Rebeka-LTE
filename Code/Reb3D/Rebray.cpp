...
// set attributs
 void RebRay::Set(RebVector vcOrig, RebVector vcDir) {
   m_vcOrig = vcOrig;
   m_vcDir  = vcDir;
   }
/*----------------------------------------------------------------*/
...
// transform ray into matrix space
 void RebRay::DeTransform(const RebMatrix &_m) {
   RebMatrix mInv;
   RebMatrix m=_m;
   // invert translation
   m_vcOrig.x -= m._41;   
...
bool RebRay::Intersects(const RebVector &vc0, const RebVector &vc1, 
                        const RebVector &vc2, bool bCull, float *t) {
   RebVector pvec, tvec, qvec;

   RebVector edge1 = vc1 - vc0;
   RebVector edge2 = vc2 - vc0;
...
bool RebRay::Intersects(const RebVector &vc0, const RebVector &vc1, 
                        const RebVector &vc2, bool bCull, float fL, float *t) {
   RebVector pvec, tvec, qvec;
...
bool RebRay::Intersects(const RebAabb &aabb, float *t) {
   bool bInside = true;
   float t0, t1, tmp;
...
// test for intersection with aabb, original code by Andrew Woo, 
// from "Geometric Tools...", Morgan Kaufmann Publ., 2002
bool RebRay::Intersects(const RebAabb &aabb, float fL, float *t) {
   bool bInside = true;
...
bool RebRay::Intersects(const RebObb &obb, float *t) {
   float e, f, t1, t2, temp;
   float tmin = -99999.9f, 
         tmax = +99999.9f;

   RebVector vcP = obb.vcCenter - m_vcOrig;
...
bool RebRay::Intersects(const RebObb &obb, float fL, float *t) {
   float e, f, t1, t2, temp;
   float tmin = -99999.9f, 
         tmax = +99999.9f;
...
bool RebRay::Intersects(const RebPlane &plane, bool bCull, 
...
bool RebRay::Intersects(const RebPlane &plane, bool bCull, float fL,
                        float *t, RebVector *vcHit) {
   float Vd = plane.m_vcN * m_vcDir;
