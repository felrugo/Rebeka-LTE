...
RebAabb::RebAabb(RebVector _vcMin, RebVector _vcMax) {
   vcMin = _vcMin;
   vcMax = _vcMax;
...
// construct from obb
void RebAabb::Construct(const RebObb *pObb) {
   RebVector vcA0, vcA1, vcA2;
   RebVector _vcMax, _vcMin;
 ...
 */
int RebAabb::Cull(const RebPlane *pPlanes, int nNumPlanes) {
   RebVector  vcMin, vcMax;
   ...
   if (bIntersects) return RebCLIPPED;
   return RebVISIBLE;
   } // Cull
/*----------------------------------------------------------------*/
...
bool RebAabb::Intersects(const RebRay &Ray, float *t) {
   bool bInside = true;
   float t0, t1, tmp;
...
bool RebAabb::Intersects(const RebAabb &aabb) {
...
bool RebAabb::Intersects(const RebVector &vc) {
...
bool RebAabb::Contains(const RebRay &Ray, float fL) {
   RebVector vcEnd = Ray.m_vcOrig + (Ray.m_vcDir*fL);
...
void RebAabb::GetPlanes(RebPlane *pPlanes) {
   RebVector vcN;
