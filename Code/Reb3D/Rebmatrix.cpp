...
/*----------------------------------------------------------------*/
 ...
 void RebMatrix::ApplyInverseRota(RebVector *pvc) {
...
/*----------------------------------------------------------------*/
 ...
 void RebMatrix::RotyByDeg(float x, float y, float z)
{
	Rota((x/180.0f) * PI, (y/180.0f) * PI, (z/180.0f) * PI);
}



 void RebMatrix::Ortho(float l, float r, float t, float b, float n, float f)
{
    _11 = 2 / (r - l);
    _12 = 0;
    _13 = 0;
 ...
 void RebMatrix::Rota(float x, float y, float z)
   { Rota(RebVector(x, y, z)); }

 void RebMatrix::Rota(const RebVector &vc) {
   ...
   _32 = (cr*sp*sy+-sr*cy);
   _33 = cr*cp;
   ori.x = (vc.x / PI) * 180.0f;
   ori.y = (vc.y / PI) * 180.0f;
   ori.z = (vc.z / PI) * 180.0f;
   } // Rota
/*----------------------------------------------------------------*/


 void RebMatrix::SetTranslation(RebVector vc, bool b) {
   if (b) Identity();
   _41 = vc.x;
   _42 = vc.y;
   _43 = vc.z;
...
/*----------------------------------------------------------------*/
 ...
 RebVector RebMatrix::GetTranslation(void)
   { return RebVector(_41, _42, _43); }
/*----------------------------------------------------------------*/


 RebVector RebMatrix::GetRotation(void)
{
	return ori;
}

 void RebMatrix::Scale(float x, float y, float z)
{
	_11 = _11 * x;
	_22 = _22 * y;
	_33 = _33 * z;
 ...
 void RebMatrix::Frustum(float left, float right, float bottom, float top, float zNear, float zFar)
{
...
// Build rotation matrix around arbitrary axis
 void RebMatrix::RotaArbi(const RebVector &_vcAxis, float a) {
...
// add translation to matrix
 void RebMatrix::Translate(float dx, float dy, float dz) {
   _41 = dx;
   _42 = dy;
   _43 = dz;
...
// look from given position at given point
 void RebMatrix::LookAt(RebVector vcPos, RebVector vcLookAt,
                              RebVector vcWorldUp) {
...
// make a billboard matrix for given position and direction
 void RebMatrix::Billboard(RebVector vcPos, RebVector vcDir,
                                 RebVector vcWorldUp) {
   RebVector vcUp, vcRight; 
   float     fAngle=0.0f;
...
// transpose the matrix
 void RebMatrix::TransposeOf(const RebMatrix &m) {
   _11 = m._11;
   _21 = m._12;
   _31 = m._13;
   _41 = m._14;
...
// invert the matrix, use of intel's SSE code is incredibly slow here.
 void RebMatrix::InverseOf(const RebMatrix &m) {
//   if (!g_bSSE) {
      RebMatrix mTrans;
