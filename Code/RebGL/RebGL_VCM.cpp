...
void RebGLVertexCache::AddBuffer(IVertexBuffer * abuff)
{
...
void RebVertexCacheManager::DeleteCache(IVertexCache * rvc)
	{
		if(rvc = 0)
			return;
		UINT CID;
	...
	IVertexCache * RebVertexCacheManager::GetVertexCache(std::string cname)
	{
	...
	IVertexCache * RebVertexCacheManager::GetVCByFile(std::string filename) /* if VC doesn't exists return 0 */
	{
		for (unsigned int i = 0; i < RVCs.size(); i++)
		{
	...
	std::vector<IVertexCache*> * RebVertexCacheManager::GetRVCs()
	{
		return &RVCs;
	}
